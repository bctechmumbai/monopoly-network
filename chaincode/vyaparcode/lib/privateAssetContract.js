...
class privateAssetContract extends Contract {
    ...
    async createAsset(ctx, jsonInputArgs) {
        ...
        // let transientJSON = transientData.asset_properties;
        ...
        let createrMSPID = cid.getMSPID();
        let collectionName=`_implicit_org_${createrMSPID}`;

        try {
            // console.log(newAsset);
            const buffer = Buffer.from(JSON.stringify(newAsset));
            // await ctx.stub.putState(record.assetId, buffer);
            await ctx.stub.putPrivateData(collectionName, record.assetId , buffer);
            let response = {
                status: 'Success',
                data: {
                    assetId: record.assetId,
    ...
    async readAsset(ctx, jsonInputArgs) {
        ...
        try {
            // const buffer = await ctx.stub.getState(record.assetId,  {privateCollection: collectionName});
            // const buffer = await ctx.stub.getPrivateData(collectionName, record.assetId);
            const buffer = await ctx.stub.getPrivateData(collectionName, record.assetId);
            ...
            let response= {
                ...
                data: {
                    assetId: record.assetId,
                    errorCode: 'ce-readAsset-READ-catch-error-422',
                    errorMessage: err.message,
                },
            };
            return response;
