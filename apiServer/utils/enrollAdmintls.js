...
async function main() {
    try {
        ...
        const caInfo = ccp.certificateAuthorities[tlscaName];
        ...
        const identity = await wallet.get(orgMSP+'_tls_'+adminName);
        if (identity) {
        ...
        await wallet.put(orgMSP+'_tls_'+adminName, x509Identity);
